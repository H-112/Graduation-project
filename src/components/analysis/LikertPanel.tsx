...
export function LikertPanel({ likert }: { likert: Record<string, DescriptiveStats[]> }) {
  ...
  return (
    <div className="space-y-8">
      <h3 className="text-lg font-bold text-gray-900">Likert 量表分析 (1-5分制)</h3>
      {entries.map(([groupName, items]) => (
        <div key={groupName} className="bg-gray-50 rounded-xl p-5">
          <h4 className="text-sm font-semibold text-gray-700 mb-4">{items[0]?.group || groupName}</h4>
              ...
              return (
                <div key={i} className="bg-white rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-medium text-gray-700">{shortLabel}</span>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="flex-1 h-2 bg-gray-100 rounded-full relative overflow-hidden">
                      ...
                      />
                      <div
                        className="absolute w-2 h-2 bg-white border-2 border-gray-700 rounded-full -translate-x-1/2 top-1/2 -translate-y-1/2"
                        style={{ left: `${(item.mean / 5) * 100}%` }}
                      />
                    </div>
                    <span className="text-[10px] text-gray-400 w-4">5</span>
                  </div>
                  ...
                  <div className="flex gap-1">
                    {item.distribution.map((d, j) => (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-gray-400">{d.score}分</span>
                      </div>
                    ))}
                  </div>
                </div>
