    ...
    <div>
      ...
      {data.cleaning && (data.cleaning.removed_empty + data.cleaning.removed_duplicates + data.cleaning.removed_low_quality + (data.cleaning.removed_skips || 0)) > 0 && (
        <div className="mt-3 flex items-center gap-3 text-xs text-gray-400">
          ...
          <span>原始: {data.cleaning.original_count}</span>
          ...
          {(data.cleaning.removed_skips || 0) > 0 && <span>· 跳过: -{data.cleaning.removed_skips}</span>}
          <span className="text-gray-600 font-medium">→ 最终: {data.cleaning.final_count}</span>
        </div>
      )}
    </div>
  );
