...
export function UsagePanel({ usage }: { usage: Record<string, CategoricalResult> }) {
  const entries = Object.entries(usage).filter(([, d]) => d?.distribution && d.distribution.length > 0);

  return (
    <div className="space-y-6">
      <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">选择题分布</h3>
          ...
          return (
            <div key={key} className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
              <div className="flex items-start justify-between gap-2 mb-3">
                <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{d.column}</h4>
                {isMulti && (
                  <span className="text-xs text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/40 rounded-full px-2 py-0.5 flex items-center gap-1 shrink-0">
                    ...
                    <div key={i} className="flex items-center gap-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between text-xs mb-0.5">
                          <span className="text-gray-600 dark:text-gray-400 truncate" title={item.label}>{item.label}</span>
